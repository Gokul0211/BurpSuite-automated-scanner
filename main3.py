...
FIXED VERSION - Proper error handling and initialization
"""
import os
import sys
import time
...
# ============================================================================
# AUTO CLICKER - WITH SAFETY FIXES
# ============================================================================
# Global rate limiting for GUI checks
LAST_GUI_CHECK = 0
GUI_CHECK_COOLDOWN = 10  # seconds

# CRITICAL: Define the region where Resume button appears
# Adjust these values based on where your Resume button is located
# Format: (x, y, width, height) - measure from top-left of screen
RESUME_REGION = (0, 0, 1200, 800)  # Adjust this to your screen layout
...
def force_unpause_gui() -> bool:
    ...
    NOW WITH SAFETY: Rate-limited + Region-restricted + Shutdown-aware
    """
    global LAST_GUI_CHECK, _ACTIVE_BURP_MANAGER
    
    # CRITICAL SAFETY: Don't attempt GUI ops during Burp shutdown
    if _ACTIVE_BURP_MANAGER and _ACTIVE_BURP_MANAGER.is_stopping:
        logging.debug("   Skipping GUI check (Burp is shutting down)")
        return False
    ...
    # Rate limiting check
    now = time.time()
    if now - LAST_GUI_CHECK < GUI_CHECK_COOLDOWN:
        logging.debug(f"   Skipping GUI check (cooldown: {GUI_CHECK_COOLDOWN}s)")
        return False
    ...
    while time.time() - start < 10:
        try:
            # FIXED: Region-limited screenshot (80-90% less GPU load)
            location = pyautogui.locateOnScreen(
                str(RESUME_BUTTON_IMG),
                region=RESUME_REGION  # CRITICAL FIX
            )
            ...
            if location:
                ...
                # Optional: Move mouse away to ensure button isn't hovered
...
# ============================================================================
# BURP PROCESS MANAGER - WITH SAFETY FIXES
# ============================================================================
    ...
    def start(self, target_url: str) -> bool:
        ...
        try:
            ...
            if GOLDEN_PROJECT.exists():
                ...
                logging.warning("⚠️  Starting with fresh project (Golden project not found)")
            
            # Burp command with official REST API + SAFETY FLAGS
            cmd = [
...
def main():
    ...
    try:
        logging.info("🔧 Initializing Burp Scanner...")
        ...
        if not os.path.exists(JAVA_PATH):
            ...
            logging.info("   Attempting to use system Java...")
            JAVA_PATH = "java"
