...
import signal
import traceback
import ctypes   # <--- ADDED: Required for Kernel Keep-Awake
# pyautogui import removed
...
# RESUME_BUTTON_IMG removed

# ============================================================================
# WINDOWS KERNEL KEEP-AWAKE LOGIC
# ============================================================================
def prevent_windows_sleep():
    """Forces Windows to stay awake and unlocked using Kernel API"""
    if os.name == 'nt':
        try:
            # ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED
            # This tells Windows: "Do not sleep, do not turn off display, do not lock."
            ctypes.windll.kernel32.SetThreadExecutionState(0x80000003)
            logging.info("✓ Windows Kernel instructed to prevent Sleep/Lock")
        except Exception as e:
            logging.warning(f"Could not set execution state: {e}")
...
class BurpConfig:
    ...
    java_path: str = "java"
    jvm_max_memory: str = "3g"  # <--- CHANGED: Set to 3g as requested
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        # Resume button image check removed
                ...
                try:
                    with open(SCAN_TEMPLATE, 'r') as f:
                        ...
                        if "scan_configurations" in template_data:
                            ...
                            logging.info(f"✓ Loaded scan_configurations from template")
                        else:
                            # Raw format detected - need to intelligently wrap it
                            logging.info("ℹ️ Detected RAW config - Auto-wrapping in CustomConfiguration")
                            
                            # Extract ONLY valid configuration keys (not urls, scope, etc.)
                            valid_config_keys = ["crawler", "scanner", "project_options"]
                            extracted_config = {
                            ...
                            if extracted_config:
                                ...
                                logging.warning("⚠️ No valid config keys found in template (expected: crawler, scanner)")
                            
                            # Also merge any other top-level keys that belong at scan level
                            # (but NOT urls/scope as we already set those)
...
def main():
    ...
    try:
        setup_logging()
        # CRITICAL FIX: Prevent Windows Sleep/Lock
    ...
    try:
        ...
        config = BurpConfig(
            ...
            java_path=JAVA_PATH,
            jvm_max_memory="3g",  # CHANGED to 3g as requested
            api_port=8090,
            api_key=""
        )
